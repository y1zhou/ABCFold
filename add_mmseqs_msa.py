...
def run_mmseqs(x, prefix, use_env=True, use_filter=True,
        ...
        host_url="https://a3m.mmseqs.com", num_templates=20) -> Tuple[List[str], List[str]]:
  ...
  if use_templates:
    ...
    for line in open(f"{path}/pdb70.m8","r"):
      ...
      if count < num_templates:
        p = line.rstrip().split()
        M, pdb, qid, alilen, qstart, qend, tstart, tend, e_value, cigar = p[0], p[1], p[2], p[3], p[6], p[7], p[8], p[9], p[10], p[12]
        coverage = float(alilen) / len(x)
        pdb_id = pdb.split("_")[0]

        # Use the same template filters as AF3 and only use 1 template per PDB
        if float(qid) == 1.0 and coverage >= 0.95 or  coverage < 0.1 or pdb_id in tested_pdbs:
          continue
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Add MMseqs2 unpaired MSA to AlphaFold3 json')
    ...
    for sequence in af3_json['sequences']:
      if 'protein' in sequence:
        ...
        if args.templates:
          a3m_lines, templates = run_mmseqs(input_sequence, "tmp", use_templates=True, num_templates=args.num_templates)
        else:
        ...
        if args.custom_template:
          ...
          template = {}
          cif_str = _get_mmcif(args.custom_template, "custom", args.custom_template_chain, 1, len(input_sequence))
